// server.js — LotProRV complete single-file server
...
db.exec(`
  ...
  CREATE VIRTUAL TABLE IF NOT EXISTS floorplans_fts USING fts5(
    make, model, floorplan, rv_type, rv_class,
    content='floorplans', content_rowid='id'
  );
  CREATE TRIGGER IF NOT EXISTS fp_ai AFTER INSERT ON floorplans BEGIN
    INSERT INTO floorplans_fts(rowid,make,model,floorplan,rv_type,rv_class)
    VALUES(new.id,new.make,new.model,new.floorplan,new.rv_type,new.rv_class);
  END;
  CREATE TRIGGER IF NOT EXISTS fp_au AFTER UPDATE ON floorplans BEGIN
    INSERT INTO floorplans_fts(floorplans_fts,rowid,make,model,floorplan,rv_type,rv_class)
    VALUES('delete',old.id,old.make,old.model,old.floorplan,old.rv_type,old.rv_class);
    INSERT INTO floorplans_fts(rowid,make,model,floorplan,rv_type,rv_class)
    VALUES(new.id,new.make,new.model,new.floorplan,new.rv_type,new.rv_class);
  END;
  CREATE TRIGGER IF NOT EXISTS fp_ad AFTER DELETE ON floorplans BEGIN
    INSERT INTO floorplans_fts(floorplans_fts,rowid,make,model,floorplan,rv_type,rv_class)
    VALUES('delete',old.id,old.make,old.model,old.floorplan,old.rv_type,old.rv_class);
  END;
...
function upsertFloorplan(data) {
  ...
  const existing = db.prepare(`SELECT id FROM floorplans WHERE source_url=?`).get(row.source_url)
  if (existing) {
    const sets = FP_FIELDS.filter(f => f !== 'source_url').map(f => `${f}=@${f}`).join(',')
    db.prepare(`UPDATE floorplans SET ${sets},updated_at=datetime('now'),is_stale=0 WHERE id=@id`)
      .run({ ...row, id: existing.id })
    return { action: 'updated', id: existing.id }
  }
  const r = db.prepare(`INSERT INTO floorplans (${FP_FIELDS.join(',')}) VALUES (${FP_FIELDS.map(f=>'@'+f).join(',')})`)
    .run(row)
...
function searchFloorplans({ q, make, year, rv_type, min_length, max_length, sleeps, slides, limit=40, offset=0 }) {
  const conds = [], params = []
  if (q) {
    const ids = db.prepare(`SELECT rowid FROM floorplans_fts WHERE floorplans_fts MATCH ? LIMIT 1000`)
      .all(q+'*').map(r=>r.rowid)
    if (!ids.length) return { rows: [], total: 0 }
    conds.push(`id IN (${ids.join(',')})`)
  }
  ...
  const total = db.prepare(`SELECT COUNT(*) as c FROM floorplans ${where}`).get(...params)?.c || 0
  const rows  = db.prepare(`
    SELECT id,year,make,model,floorplan,rv_type,rv_class,
           length_ft,dry_weight_lbs,gvwr_lbs,sleep_capacity,slides_count,msrp,
           parse_confidence,scraped_at
...
function bulkUpsertUrls(items) {
  const stmt = db.prepare(`INSERT INTO urls (url,type,discovered_from,priority) VALUES (@url,@type,@discoveredFrom,@priority) ON CONFLICT(url) DO NOTHING`)
  db.transaction(rows => rows.forEach(r => stmt.run(r)))(items)
}

function claimNextUrl() {
  return db.prepare(`
    UPDATE urls SET status='in_progress', updated_at=datetime('now')
    WHERE id=(SELECT id FROM urls WHERE status='pending' AND (next_crawl_at IS NULL OR next_crawl_at<=datetime('now')) ORDER BY priority DESC, id ASC LIMIT 1)
    RETURNING *
  `).get()
}

function markUrlDone(id, s) {
  db.prepare(`UPDATE urls SET status='done',http_status=?,last_crawled_at=datetime('now'),next_crawl_at=datetime('now','+7 days'),error=NULL,updated_at=datetime('now') WHERE id=?`).run(s, id)
}

function markUrlError(id, s, e) {
  db.prepare(`UPDATE urls SET http_status=?,error=?,retry_count=retry_count+1,status=CASE WHEN retry_count>=4 THEN 'error' ELSE 'pending' END,next_crawl_at=datetime('now','+5 minutes'),updated_at=datetime('now') WHERE id=?`).run(s, String(e).slice(0,500), id)
}

function startCrawlRun(trigger, scope) {
  return db.prepare(`INSERT INTO crawl_runs (trigger,scope) VALUES (?,?) RETURNING id`).get(trigger, scope)?.id
}

function finishCrawlRun(id, stats) {
  db.prepare(`UPDATE crawl_runs SET status=@status,urls_crawled=@crawled,urls_failed=@failed,records_added=@added,records_updated=@updated,finished_at=datetime('now') WHERE id=@id`)
    .run({ id, status:'done', ...stats })
}

function addLog(runId, level, message, url=null) {
  try { db.prepare(`INSERT INTO crawl_logs (run_id,level,message,url) VALUES (?,?,?,?)`).run(runId, level, String(message).slice(0,2000), url) } catch {}
}

// ── SCRAPER / PARSERS ─────────────────────────────────────────────────────────

const BASE = 'https://www.rvusa.com'
const abs = href => !href ? null : href.startsWith('http') ? href : BASE+(href.startsWith('/')?href:'/'+href)
...
const FIELD_MAP = [
  ...
  { key:'gvwr_lbs',         labels:['gvwr','gross vehicle weight','gross weight rating'] },
  { key:'hitch_weight_lbs', labels:['hitch weight','tongue weight'] },
  { key:'pin_weight_lbs',   labels:['pin weight','kingpin'] },
  { key:'payload_lbs',      labels:['payload','cargo capacity','net carrying capacity'] },
...
function parsePage(html, url) {
  ...
  // Parse floorplan specs
  const rawSpecs = {}
  const tableSelectors = ['table.specs-table','table[class*="spec"]','.rv-specs table','table']
  ...
  if (Object.keys(rawSpecs).length<4) {
    $('dl').each((_,dl) => {
      ...
      dts.forEach((dt,i)=>{ const k=$(dt).text().trim(),v=dds[i]?$(dds[i]).text().trim():null; if(k&&v) rawSpecs[k]=v })
    })
  }
  ...
  return {
    ...
    source_url:url, year, make:titleCase(make), model:titleCase(model),
    floorplan:titleCase(floorplan), rv_type, rv_class, ...mapped,
    raw_specs:Object.keys(rawSpecs).length?rawSpecs:null,
    parse_confidence, parse_notes:notes.length?notes.join('; '):null,
  }
}
...
async function fetchPage(url, scraperOpts={}) {
  ...
  }, { retries:4, minTimeout:1500, maxTimeout:32000, factor:2,
...
async function runCrawl({ trigger='manual', scope=null, seedUrls=null }={}) {
  ...
  log('info',`Crawl #${runId} started — ${trigger} ${scope||''}`)
  ...
  finishCrawlRun(runId,{crawled:stats.crawled,failed:stats.failed,added:stats.added,updated:stats.updated})
  const c=activeCrawls.get(runId); if(c) c.status='done'
  console.log(`✅ Crawl #${runId} done — added=${stats.added} crawled=${stats.crawled} failed=${stats.failed}`)
  return {runId,...stats}
}
...
app.get('/api/floorplans', (req,res) => {
  try {
    res.json(searchFloorplans({
      ...
      max_length:req.query.max_length, sleeps:req.query.sleeps,
      slides:req.query.slides,
      limit:Math.min(+(req.query.limit||40),100),
...
app.get('/api/meta', (req,res) => {
  res.json({
    makes: db.prepare(`SELECT DISTINCT make FROM floorplans WHERE make IS NOT NULL ORDER BY make`).all().map(r=>r.make),
    rv_types: db.prepare(`SELECT DISTINCT rv_type FROM floorplans WHERE rv_type IS NOT NULL ORDER BY rv_type`).all().map(r=>r.rv_type),
  })
})
...
app.get('/api/stats', (req,res) => {
  res.json({
    floorplans: {
      total:   db.prepare(`SELECT COUNT(*) as c FROM floorplans`).get().c,
      byType:  db.prepare(`SELECT rv_type,COUNT(*) as c FROM floorplans GROUP BY rv_type ORDER BY c DESC`).all(),
      byMake:  db.prepare(`SELECT make,COUNT(*) as c FROM floorplans GROUP BY make ORDER BY c DESC LIMIT 20`).all(),
      lowConf: db.prepare(`SELECT COUNT(*) as c FROM floorplans WHERE parse_confidence<0.5`).get().c,
    },
    urls: db.prepare(`SELECT type,status,COUNT(*) as count FROM urls GROUP BY type,status ORDER BY type,status`).all(),
  })
})
...
app.get('/api/admin/crawl/stream', requireAdmin, (req,res) => {
  ...
  const iv = setInterval(()=>{
    let latest=null
    for (const [runId,crawl] of activeCrawls) if(!latest||runId>latest.runId) latest={runId,...crawl}
...
app.get('/api/admin/errors',          requireAdmin,(req,res)=>res.json(db.prepare(`SELECT * FROM urls WHERE status='error' ORDER BY updated_at DESC LIMIT ?`).all(+(req.query.limit||100))))
app.post('/api/admin/requeue-errors', requireAdmin,(req,res)=>{ db.prepare(`UPDATE urls SET status='pending',retry_count=0,error=NULL,updated_at=datetime('now') WHERE status='error'`).run(); res.json({ok:true}) })
app.get('/api/admin/logs',            requireAdmin,(req,res)=>res.json(db.prepare(`SELECT l.*,r.trigger,r.scope FROM crawl_logs l LEFT JOIN crawl_runs r ON l.run_id=r.id ORDER BY l.id DESC LIMIT ?`).all(+(req.query.limit||300))))
app.get('/api/admin/runs',            requireAdmin,(req,res)=>res.json(db.prepare(`SELECT * FROM crawl_runs ORDER BY id DESC LIMIT 25`).all()))
...
<script>
const API='/api';
let page='browse',selId=null,adminPw='',crawling=false;
let filters={q:'',make:'',year:'',rv_type:'',min_length:'',max_length:'',sleeps:'',slides:''};
let bPage=0,bTotal=0,metaCache={makes:[],rv_types:[]};
const PAGE_SZ=40;
...
function renderNav(){
  ...
  document.getElementById('nav').innerHTML=
    navBtn('browse','Browse',page==='browse'||page==='detail')+
...
async function showPage(p,id){
  ...
  else if(p==='admin-auth')renderAdminAuth();
}
...
async function renderBrowse(){
  ...
  if(!metaCache.makes.length){
    try{const d=await apiFetch('/meta');metaCache=d;}catch{}
  }
  m.innerHTML=\`
  <div style="padding:36px 0 24px;border-bottom:1px solid var(--border);margin-bottom:24px">
    <h1 style="font-family:'Bebas Neue',sans-serif;font-size:52px;letter-spacing:3px;line-height:1;margin-bottom:4px">FIND YOUR <span style="color:var(--accent)">RIG</span></h1>
  ...
  <div style="display:flex;gap:10px;flex-wrap:wrap;margin-bottom:20px;align-items:flex-end">
    ...
    \${fgrp('Min Length',\`<input type="number" value="\${filters.min_length}" placeholder="ft" style="width:80px;padding:7px 10px;font-size:13px" oninput="filters.min_length=this.value;debSearch()">\`)}
    \${fgrp('Max Length',\`<input type="number" value="\${filters.max_length}" placeholder="ft" style="width:80px;padding:7px 10px;font-size:13px" oninput="filters.max_length=this.value;debSearch()">\`)}
...
async function doSearch(newOffset){
  if(newOffset!==undefined)bPage=Math.floor(newOffset/PAGE_SZ);
...
function renderResults({rows,total}){
  ...
  let html=\`<div style="font-size:13px;color:var(--text3);margin-bottom:14px"><strong style="color:var(--text)">\${total.toLocaleString()}</strong> floorplans\${total===0?' — try running a crawl from Admin':''}</div>\`;
  if(!rows.length){
    html+=\`<div style="text-align:center;padding:80px 0;color:var(--text3)"><div style="font-size:48px;margin-bottom:12px">🚐</div><div style="font-size:20px;color:var(--text);margin-bottom:8px">No results</div><p style="font-size:13px">Adjust filters or run a crawl from the Admin panel</p></div>\`;
  } else {
    ...
    rows.forEach((fp,i)=>{
      ...
      html+=\`<div onclick="showPage('detail',\${fp.id})" class="fade" style="background:var(--surf);border:1px solid var(--border);border-radius:var(--r);padding:16px;cursor:pointer;transition:all .15s;animation-delay:\${i*20}ms;position:relative;overflow:hidden" onmouseenter="this.style.borderColor='var(--accent)';this.style.transform='translateY(-2px)'" onmouseleave="this.style.borderColor='var(--border)';this.style.transform='none'">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;border-top:1px solid var(--border);padding-top:10px">
          \${sm('Length',fp.length_ft?fp.length_ft+"'":null)}\${sm('Dry Wt',fp.dry_weight_lbs?num(fp.dry_weight_lbs)+' lbs':null)}
          \${sm('Sleeps',fp.sleep_capacity)}\${sm('MSRP',fp.msrp?'$'+num(fp.msrp):null)}
        </div>
  ...
  if(pages>1){
    ...
    if(bPage>0)html+=pBtn('← Prev',(bPage-1)*PAGE_SZ);
    for(let i=Math.max(0,bPage-2);i<=Math.min(pages-1,bPage+2);i++)
      html+=\`<button onclick="doSearch(\${i*PAGE_SZ})" style="padding:5px 12px;border-radius:var(--r-sm);border:none;background:\${i===bPage?'var(--accent)':'var(--surf2)'};color:\${i===bPage?'#000':'var(--text)'};font-size:12px">\${i+1}</button>\`;
    if(bPage<pages-1)html+=pBtn('Next →',(bPage+1)*PAGE_SZ);
    html+=\`</div>\`;
  }
  el.innerHTML=html;
}
...
async function renderDetail(id){
  ...
  try{
    ...
    const raw=typeof fp.raw_specs==='object'&&fp.raw_specs?fp.raw_specs:{};
    m.innerHTML=\`<div class="fade">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(280px,1fr));gap:16px">
        ...
        <div style="grid-column:1/-1;font-size:11px;color:var(--text3);font-family:var(--mono)">Source: <a href="\${fp.source_url}" target="_blank" style="color:var(--accent)">\${fp.source_url}</a> · Scraped \${fp.scraped_at?.slice(0,10)||'unknown'}</div>
      </div>
    </div>\`;
  }catch(e){m.innerHTML=\`<div style="padding:40px;text-align:center;color:var(--err)">Error: \${esc(e.message)}</div>\`}
}
...
function sCard(title,rows){
  ...
  return \`<div style="background:var(--surf);border:1px solid var(--border);border-radius:var(--r);overflow:hidden">
    ...
    \${rows.map(([l,v,f])=>\`<div style="display:flex;justify-content:space-between;align-items:center;padding:8px 16px;border-bottom:1px solid var(--border);gap:16px"><span style="font-size:12px;color:var(--text3)">\${l}</span><span style="font-family:var(--mono);font-size:12px;font-weight:500;color:\${v!=null?'var(--text)':'var(--text3)'};font-style:\${v==null?'italic':'normal'}">\${v!=null?(f?f(v):v):'N/A'}</span></div>\`).join('')}
  </div>\`;
}

// ── ADMIN ─────────────────────────────────────────────────────────────────────
function renderAdminAuth(){
  document.getElementById('main').innerHTML=\`<div class="fade" style="max-width:360px;margin:70px auto;background:var(--surf);border:1px solid var(--border);border-radius:var(--r);padding:32px;text-align:center">
...
async function tryLogin(){
  ...
  try{await apiFetch('/admin/auth-check',{headers:{'X-Admin-Password':v}});adminPw=v;showPage('admin')}
...
async function renderAdmin(){
  ...
  const m=document.getElementById('main');
  let stats={floorplans:{total:0,byType:[],byMake:[],lowConf:0},urls:[]};
  let runs=[],errors=[],logs=[];
  ...
  const urlCounts={};stats.urls.forEach(u=>{urlCounts[u.status]=(urlCounts[u.status]||0)+u.count});
  ...
  m.innerHTML=\`<div class="fade">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:10px;margin-bottom:28px">
      \${tile('Floorplans',stats.floorplans.total,'var(--accent)')}\${tile('Low Conf',stats.floorplans.lowConf,'var(--warn)')}\${tile('Makes',stats.floorplans.byMake.length,'var(--info)')}\${tile('URL Done',urlCounts.done||0,'var(--ok)')}\${tile('Pending',urlCounts.pending||0,'var(--text3)')}\${tile('Errors',urlCounts.error||0,'var(--err)')}
    </div>
    <div style="display:flex;gap:0;border-bottom:1px solid var(--border);margin-bottom:20px" id="adm-tabs">
      \${aTab('ov','Overview',true)}\${aTab('cr','Crawl',false)}\${aTab('er','Errors ('+errors.length+')',false)}\${aTab('lg','Logs',false)}
    </div>
    <div id="adm-body"></div>
  </div>\`;

  window._adminData={stats,runs,errors,logs,maxT,maxM};
  showATab('ov');
  startSSE();
}
...
function showATab(id){
  ...
  const {stats,runs,errors,logs,maxT,maxM}=window._adminData||{};
  ...
  if(id==='ov'){
    body.innerHTML=\`<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
      \${surf('By RV Type',(stats?.floorplans?.byType||[]).map(r=>bRow(r.rv_type,r.c,maxT)).join(''))}
      \${surf('Top Makes',(stats?.floorplans?.byMake||[]).map(r=>bRow(r.make,r.c,maxM)).join(''))}
    </div>\`;
  } else if(id==='cr'){
    body.innerHTML=\`<div style="display:grid;grid-template-columns:1fr 1fr;gap:16px">
      \${surf('Trigger Crawl',\`<div style="display:flex;flex-direction:column;gap:12px">
        <button onclick="triggerCrawl('full')" style="background:var(--accent);color:#000;border:none;border-radius:var(--r-sm);padding:10px;font-weight:600;font-size:13px">▶ Full Crawl — All Makes</button>
      ...
      \${surf('Run History',runs.map(r=>\`<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;background:var(--surf2);border-radius:var(--r-sm);margin-bottom:6px;flex-wrap:wrap">
        ...
        <span style="font-size:12px;flex:1">\${r.trigger}\${r.scope?' — '+r.scope:''}</span>
        <span style="font-size:11px;color:var(--text3)">+\${r.records_added} added · \${r.urls_crawled} crawled</span>
      </div>\`).join('')||'<div style="color:var(--text3);font-size:13px;text-align:center;padding:20px">No runs yet</div>',null,'1/-1')}
    </div>\`;
  } else if(id==='er'){
    body.innerHTML=surf('Error URLs',errors.length?
      \`<div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:12px"><thead><tr>\${['URL','Type','HTTP','Retries','Error'].map(h=>\`<th style="padding:8px 12px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:1px;color:var(--text3);background:var(--surf2);border-bottom:1px solid var(--border)">\${h}</th>\`).join('')}</tr></thead><tbody>\${errors.map(r=>\`<tr style="border-bottom:1px solid var(--border)"><td style="padding:8px 12px;max-width:240px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap"><a href="\${r.url}" target="_blank" style="color:var(--accent)">\${r.url}</a></td><td style="padding:8px 12px">\${r.type}</td><td style="padding:8px 12px">\${r.http_status||'—'}</td><td style="padding:8px 12px">\${r.retry_count}</td><td style="padding:8px 12px;color:var(--err);max-width:200px">\${esc(r.error||'')}</td></tr>\`).join('')}</tbody></table></div>\`
      :\`<div style="text-align:center;padding:32px;color:var(--text3)">🎉 No errors</div>\`);
  } else if(id==='lg'){
    body.innerHTML=surf('Recent Logs',logs.length?
      logs.map(l=>{const col={info:'var(--text2)',warn:'var(--warn)',error:'var(--err)',debug:'var(--text3)'}[l.level];return\`<div style="display:grid;grid-template-columns:52px 44px 1fr;gap:8px;padding:5px 12px;border-bottom:1px solid var(--border);font-size:11px;font-family:var(--mono)"><span style="color:var(--text3)">\${(l.created_at||'').slice(11,16)}</span><span style="color:\${col}">\${l.level.toUpperCase()}</span><span style="word-break:break-all">\${esc(l.message)}\${l.url?\`<span style="display:block;color:var(--text3);font-size:10px">\${esc(l.url)}</span>\`:''}</span></div>\`}).join('')
      :\`<div style="color:var(--text3);font-size:13px;text-align:center;padding:20px">No logs yet</div>\`);
  }
}
...
function bRow(label,val,max){const p=Math.round(val/max*100);return \`<div style="margin-bottom:8px"><div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:3px"><span style="color:var(--text2)">\${esc(label||'Unknown')}</span><span style="font-family:var(--mono);font-size:11px">\${val}</span></div><div style="height:4px;background:var(--surf3);border-radius:99px;overflow:hidden"><div style="width:\${p}%;height:100%;background:var(--accent);border-radius:99px"></div></div></div>\`}
